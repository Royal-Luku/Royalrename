...
async def send_doc(client,message):
       ...
       if update_channel :
       	try:
       		...
       		reply_to_message_id = message.id,
       		reply_markup = InlineKeyboardMarkup(
       		[ [ InlineKeyboardButton("Click Here" ,url=f"https://t.me/{update_channel}") ]   ]))
       		return
       try:
           ...
           await message.reply_text("**__Some Error Coming From Server Press /Start__**")
       try:
       ...
       if left > 0:       	    
       	await message.reply_text(f"**Flood Control Is Active Wait Some Times**```{ltime}```",reply_to_message_id = message.id)
       else:
       		...
       		if value < file.file_size:
       		    if STRING:
       		        ...
       		        if pre_check == True:
       		            await message.reply_text(f"""**What do you want me to do with this file?**\n\n**📁 File Name** :- {filename}\n\n**⚙️ File Size ** :- {humanize.naturalsize(file.file_size)}\n\n**📡 Data Center** :- {dcid}""",reply_to_message_id = message.id,reply_markup = InlineKeyboardMarkup([[ InlineKeyboardButton("📝 Rename File ",callback_data = "rename"),InlineKeyboardButton("✖️ Cancel",callback_data = "cancel")  ]]))
       		            ...
       		            return
       		          	...
       		          	await message.reply_text("File Bigger Than 2GB")
       		          	return
       		else:
       		    if buy_date:
       		        pre_check = check_expi(buy_date)
